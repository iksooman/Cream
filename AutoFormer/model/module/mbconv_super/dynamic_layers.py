...
class DynamicMBConvLayer(MyModule):
    ...
    def __init__(self, in_channel_list, out_channel_list,
                 kernel_size_list=3, expand_ratio_list=3, stride=1, act_func='relu', use_se=True, channels_per_group=1):
    ...
    def compute_active_subnet_flops(self, channel):
        raise NotImplementedError
        '''
        flops =  channel*channel*max(self.expand_ratio_list)
        flops += channel*max(self.expand_ratio_list)*9
        flops += channel*max(self.expand_ratio_list)*channel

        return flops
        '''

    def calc_sampled_param_num_conv(self):
        expanded_channel = self.active_out_channel*self.active_expand_ratio

        # conv params
        params =  self.active_out_channel*expanded_channel
        params += expanded_channel*self.active_kernel_size*self.active_kernel_size
        params += expanded_channel*self.active_out_channel

        # batch params
        params += expanded_channel*4
        params += expanded_channel*4
        params += self.active_out_channel*4

        # se params
        if self.use_se:
            params += 2*expanded_channel*make_divisible(expanded_channel//4, divisor=8)
    ...
    def set_subnet_config(self, in_channel, active_out_channel=None, active_kernel_size=None, active_expand_ratio=None, preserve_weight=True):
        if active_out_channel is not None:
            self.active_out_channel = active_out_channel
        else:
            self.active_out_channel = in_channel
        if active_kernel_size is not None:
            self.active_kernel_size = active_kernel_size
        if active_expand_ratio is not None:
            self.active_expand_ratio = active_expand_ratio

    def get_active_subnet(self, in_channel, active_out_channel=None, active_kernel_size=None, active_expand_ratio=None, preserve_weight=True):
        pass
        '''
