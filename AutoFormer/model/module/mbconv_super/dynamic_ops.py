...
class DynamicBatchNorm2d(nn.Module):
    ...
    def __init__(self, max_feature_dim, switchable_bn=True):
        ...
        '''''' # temp code
        channel = [192, 216, 240, 1152, 1296, 1440]
        for ch in channel:
            self.bn[str(ch)] = nn.BatchNorm2d(ch).to('cuda')
        '''''' # temp code
    ...
    def forward(self, x):
        ...
        if self.switchable_bn:
            try:
                bn = self.bn[str(feature_dim)]
            except:
                pass
                '''
                # self.bn.update([str(feature_dim), nn.BatchNorm2d(feature_dim).to(x.device)])
                self.bn[str(feature_dim)] = nn.BatchNorm2d(feature_dim).to(x.device)
                bn = self.bn[str(feature_dim)]
                '''
        else:
            bn = self.bn
